...
void main() {
  ...
  testWidgets(
      'Home lesson availability is derived from Energy, capped at the '
  ...
  testWidgets('Home lesson start opens the matching Lessons tab popup',
    ...
    await tester.pump();
    expect(
        find.byKey(const Key('lesson-start-popup-play-button')), findsNothing);
    expect(tester.takeException(), isNull);
  });
  ...
  testWidgets('claiming a lesson unlocks only its matching Home sticker',
    ...
    HomeStickerGrid grid() => tester.widget<HomeStickerGrid>(
          find.byType(HomeStickerGrid),
        );
