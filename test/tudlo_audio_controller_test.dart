...
import 'package:tudlo/shared/audio/audio_backend.dart';
...
void main() {
  ...
  test('shared button taps honor the Sound Effects settings', () async {
    ...
    expect(
      backend.commands,
      contains(
        'play:${TudloAudioAssets.buttonTapSoundEffect}:15.0:false',
      ),
    );
    final playCount =
    ...
    await controller.applySettings(
      AppSettings.defaults.copyWith(sfxVolume: 0),
    );
    controller.playButtonTapSound();
    await Future<void>.delayed(Duration.zero);
    expect(
  ...
  test('a button can override the shared effect for a meaningful action', () async {
    final backend = _FakeAudioBackend();
    final controller = TudloAudioController(backend: backend);
    addTearDown(controller.dispose);
    ...
    expect(
      backend.commands,
      contains(
        'play:${TudloAudioAssets.mapUnlockedSoundEffect}:15.0:false',
      ),
    );
  });
  ...
  test('a stopped awaited voice-over always releases its screen flow',
      () async {
    final backend = _FakeAudioBackend();
    final controller = TudloAudioController(backend: backend);
    addTearDown(controller.dispose);
  ...
  test('voice-over ducks music and restores its Settings volume afterward',
      () async {
    final backend = _FakeAudioBackend();
    final controller = TudloAudioController(backend: backend);
    addTearDown(controller.dispose);
...
class _FakeAudioBackend implements TudloAudioBackend {
  ...
  @override
  Future<void> waitForPlaybackEnd(
    Object source,
