...
import 'package:tudlo/features/lesson/domain/lesson_content.dart';
...
void main() {
  ...
  test('catalog contains the 12 approved flows in guided grade order', () {
    ...
    expect(
      LessonCatalog.forGrade(1).map((lesson) => lesson.id),
      ['g1_u1_l1', 'g1_u1_l7', 'g1_u2_l1', 'g1_u2_l4'],
    );
    expect(
      LessonCatalog.forGrade(2).map((lesson) => lesson.id),
      ['g2_u1_l1', 'g2_u1_l2', 'g2_u2_l1', 'g2_u2_l2'],
    );
    expect(
      LessonCatalog.forGrade(3).map((lesson) => lesson.id),
      ['g3_u1_l1', 'g3_u1_l3', 'g3_u2_l1', 'g3_u2_l2'],
    );
  });

  test('catalog location mapping and Grade 3 orientation are stable', () {
    expect(
      LessonCatalog.forLocation(grade: 1, location: MapLocation.school)
          .map((lesson) => lesson.id),
      ['g1_u1_l1', 'g1_u1_l7'],
    );
    expect(
      LessonCatalog.forLocation(grade: 2, location: MapLocation.plaza)
          .map((lesson) => lesson.id),
      ['g2_u2_l1', 'g2_u2_l2'],
    );
    expect(
      LessonCatalog.forLocation(grade: 3, location: MapLocation.market)
          .map((lesson) => lesson.id),
      ['g3_u1_l1', 'g3_u1_l3', 'g3_u2_l2'],
    );
    expect(
      LessonCatalog.forGrade(3)
          .every((lesson) => lesson.orientation == LessonOrientation.landscape),
      isTrue,
    );
    expect(
      LessonCatalog.all.map((lesson) => lesson.rewardAsset).every(
          (asset) => asset.startsWith('assets/images/stickers/rewards/home/')),
      isTrue,
    );
  });

  test('map defaults keep House as a chooser and mapped locations as catalogs',
      () {
    expect(
  ...
  test(
    ...
    await learner.seedLessonProgress(
      activeLessonId: 'g1_u1_l1',
    );
    ...
    expect(afterFirst.unlockedMapLocations,
    ...
    expect(afterSeventh.lessonProgress.completions[seventh.id]!.rewardAsset,
  ...
  test(
      'active lesson restores from a persisted learner after restart, '
      'with no map event', () async {
    final original = LearnerController();
    await original.createAndSave(name: 'Koka', grade: 3, energy: 60);
    await original.seedLessonProgress(
      activeLessonId: 'g3_u1_l1',
    );
  ...
  test('preserved source catalogue receives only a selected location’s levels',
      () {
  ...
  testWidgets(
    ...
    DevGLessonHostScope.of(tester.element(find.byType(LessonsScreen)))
        .startSourceLevel(1);
    await tester.pump();
    expect(find.byType(LevelGamePage), findsOneWidget);
    expect(find.byType(AppBottomTabNavigation), findsNothing);

    await DevGLessonHostScope.of(tester.element(find.byType(LevelGamePage)))
        .exitIncomplete();
    await tester.pump();
    expect(find.byType(LessonsScreen), findsOneWidget);
    expect(find.byType(AppBottomTabNavigation), findsOneWidget);
  ...
  testWidgets('continuing after lesson one opens the next lesson popup',
    ...
    expect(find.byType(LessonsScreen), findsOneWidget);
    expect(find.byKey(const Key('lesson-start-popup-play-button')),
        findsOneWidget);
    expect(find.byType(AppBottomTabNavigation), findsOneWidget);
  ...
  testWidgets('map-launched lesson continues into the Lessons gallery popup',
    ...
    DevGLessonHostScope.of(tester.element(find.byType(LessonIntroPage)))
        .startSourceLevel(1);
    await tester.pump();
