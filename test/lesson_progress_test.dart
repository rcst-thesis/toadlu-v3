...
void main() {
  ...
  test('catalog location mapping and Grade 3 orientation are stable', () {
    ...
    expect(
      LessonCatalog.forLocation(grade: 3, location: MapLocation.school)
          .map((lesson) => lesson.id),
      ['g3_u2_l2'],
    );
    expect(
  ...
  testWidgets('a Grade 3 learner opens the Grade 3 lesson catalogue',
      (tester) async {
    final learner = LearnerController();
    await learner.createAndSave(name: 'Koka', grade: 3, energy: 100);
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: LearnerScope(
          ...
          child: LessonProgressScope(
            controller: controller,
            child: const DevGLessonCatalogHost(),
          ),
        ),
      ),
    );
