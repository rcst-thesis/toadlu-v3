...
void main() {
  ...
  group('resolveWordOfTheDay', () {
    ...
    test('a different calendar date picks again even with a valid stored id',
        () {
      final selection = resolveWordOfTheDay(
        pool: _pool,
        storedId: 'a',
  ...
  group('resolveFeatured', () {
    test('same calendar date + all stored ids still in pool returns cached',
        () {
      final now = DateTime(2026, 1, 15, 10);
    ...
    test(
        'favors the highest-count category more often than not (weighted, '
  ...
  group('LearnerController featured/category mutators', () {
    test('incrementCategorySearchCount is a no-op with no current learner',
        () async {
      final controller = LearnerController();
