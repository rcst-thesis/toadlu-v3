...
class TudloAudioController extends ChangeNotifier {
  TudloAudioController({TudloAudioBackend? backend})
    ...
    _splashSting = _OneShotAudio(
      _backend,
      TudloAudioAssets.maralSplashSting,
    );
  }
  ...
  void playButtonTapSound({String? soundEffectAsset}) {
    if (_disposed) return;
    unawaited(
      playSoundEffect(soundEffectAsset ?? TudloAudioAssets.buttonTapSoundEffect),
    );
  }
  ...
  Future<_VoiceOverSession?> _startVoiceOver(String assetPath) async {
    ...
    final playback =
        await _voiceOverFor(assetPath).play(_settings, _AudioChannel.voiceOver);
  ...
  _OneShotAudio _soundEffectFor(String assetPath) =>
      _soundEffects.putIfAbsent(
        assetPath,
        () => _OneShotAudio(_backend, assetPath),
      );
