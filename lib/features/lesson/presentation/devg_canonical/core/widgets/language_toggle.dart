...
class TudloVoiceButton extends StatelessWidget {
  ...
  static Future<void> speak(
    ...
    try {
      ...
      // DevG's optional text-to-speech fallback is intentionally not ported.
      // Supplied recorded lesson clips still use Tudlo's voice-over channel;
      // an unmatched helper phrase remains visual rather than creating a
      // second audio stack or ignoring the learner's voice setting.
      if (waitForCompletion) {
        if (_speechToken == token) isSpeaking.value = false;
        await audio.restoreBackgroundVolume();
      } else {
