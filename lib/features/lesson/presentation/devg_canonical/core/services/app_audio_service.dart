...
class AppAudioService {
  ...
  Future<void> playLessonComplete() => playSoundEffect(lessonComplete, volume: .48);
  Future<void> playStar() => playSoundEffect(star, volume: .38);

  Future<void> playVoiceAssets(List<String> paths, {double volume = .95}) async {
    await stopVoice();
  ...
  Future<void> stopVoice() async {
    ...
    await Future.wait(active.map((asset) => _audio?.stopVoiceOver(asset) ?? Future.value()));
  }
